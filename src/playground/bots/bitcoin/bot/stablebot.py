...
class StableBot(object):
    def __init__(self, max_btc, max_usd, init_action, init_price, trigger_percent):
        print now(), 'stablebot:init_bot', max_btc, max_usd, init_action, init_price, trigger_percent
    ...
    def run_once(self):
        ...
        if self.next_action=='sell':
            ...
            print now(), 'stablebot:run_once', my_btc, my_usd, current_price, self.next_action, self.next_price
            amount = min(self.max_btc, my_btc)
            if current_price>=self.next_price or random.random()<=0.01:
                print now(), 'stablebot:begin sell ', amount
                print 'stablebot:sell result', sell(amount)
                self.next_action = 'buy'
                self.next_price = int(current_price*(1-self.trigger_percent))
                print now(), 'stablebot:sell ', amount
            ...
            if current_price<=self.next_price or random.random()>=0.99:
                print now(), 'stablebot:begin buy', amount
                print 'stablebot:buy result', buy(amount)
                self.next_action = 'sell'
                self.next_price = int(current_price*(1+self.trigger_percent))
                print now(), 'stablebot:buy', amount
    ...
    def run(self):
        while 1:
            ...
            print 'stablebot:Sleeping for '+str(mysleep)+' seconds...'            
            time.sleep(mysleep)
