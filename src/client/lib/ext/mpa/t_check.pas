...
var
  i,f: integer;
begin
  writeln('mp_coshmult test: ', fn_coshm);
...
const
  AF = 9;
  fn: array[1..AF] of word = (0,1,2,3,255,256,1111,3333,5555);
var
  f,i: integer;
  n  : word;
begin
  ...
  for i:=1 to AF do begin
    n := fn[i];
    {$ifdef BIT16}
      if n>$7FF then n := n and $7FF;
    {$endif}
      mp_fact0(n,a);
      mp_fact(n,b);
      if (mp_error<>MP_OKAY) or mp_is_ne(a,b) then inc(f);
      write('.');
  end;
  writeln;
  writeln('No. of checks: ',AF,', failed: ',f);
  inc(totalfailed,f);
  writeln;
end;
...
begin
  ...
  ps := mp_numth.mp_show_progress;
  mp_numth.mp_show_progress := false;
  pfd_initialize(ctx);
  mp_fact(36,a);
  mp_inc(a);
  pfd_factor(ctx, a);
...
{$ifdef MP_32BIT}
  TV: array[-1..1] of longint = ($1A0004,$40001,$140003);  {4 byte mp_digits}
{$else}
  TV: array[-1..1] of longint = ($120004,$40001,$E0003);   {2 byte mp_digits}
{$endif}
var
  i,f: integer;
  A1, AF: longint;
...
{Calculated with: CALC - a number theory calculator, K.R.Matthews, V 2008-03-13}
const
...
begin
  ...
  cnt1:= 0;
  cnt4:= 0;
  writeln('Test mp_cornacchia/mp_cornacchia4: ');
  for i:=1 to 5 do begin
    inc(cnt);
  ...
  writeln;
  writeln('No. of checks: ',cnt,'+',cnt1,'+',cnt4,', failed: ',f);
  inc(totalfailed,f);
  writeln;
end;
...
begin
  writeln('Test of MPArith V', MP_VERSION, ' [main test]  (c) 2004-2012 W.Ehrhardt ');
