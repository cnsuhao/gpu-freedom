...
(*-------------------------------------------------------------------------
 (C) Copyright 2005-2011 Wolfgang Ehrhardt
...
uses
  BTypes,mp_base,mp_prng,mp_numth;
...
begin
  ...
  if mp_error=MP_OKAY then begin
    ...
    repeat
      ...
      mp_rand_bits(p,s4);
      {$ifdef MPC_TRACE}
        if mp_bitsize(p)<>s4 then mp_tracevv('!');
      {$endif}
      ...
      if mp_gcd1(e,t,t) then begin
        {$ifdef MPC_TRACE}
          mp_tracevvv('o');
        {$endif}
        {done if p is a BPSW pseudo prime}
        if mp_is_spsp_d(p,2) and mp_is_slpsp(p) then break;
      end;
    until false;
    {$ifdef MPC_TRACE}
      mp_tracevv('  p done'#13#10);
    {$endif}
    ...
    repeat
      ...
      mp_sub_d(q,1,t);
      if mp_gcd1(e,t,t) then begin
        {done if q is a BPSW pseudo prime}
        {$ifdef MPC_TRACE}
          mp_tracevvv('.');
        {$endif}
