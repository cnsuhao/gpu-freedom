...
(*-------------------------------------------------------------------------
 (C) Copyright 2007-2012 Wolfgang Ehrhardt
...
begin
  ...
  a.exponent := longint(txr(x).sexp and $7fff) -($3fff+63);
  s_mpf_normalize(a);
end;
...
begin
  ...
  {$endif}
  if mpf_is0(a) or (s_mpf_ldx(a) < -(b.bitprec+32) div 2) then begin
...
begin
  if mpf_cmp_mag_ext(a,double(MaxDblHex))>0 then begin
...
begin
  if mpf_cmp_mag_ext(a,extended(MaxExtHex))>0 then begin
...
begin
  ...
  mpf_initp5(e,q,w,x,z,b.bitprec+32);
  pw := w.bitprec;
  mpf_copyp(a,x);
  ...
  if (bsa <= 1020) or s_mpf_is_neg(a) then begin
    ...
    if t=-1.0 then begin
      ...
      s_mpf_inc1(w);
      mpf_mul_2k(w,1,w);
      if s_mpf_is_neg(w) then mpf_set_ext(w,-1)
      else begin
        {z = p}
        mpf_sqrt(w,z);
        mpf_div_ext(w,-3.0,w);
      ...
      {$ifdef MPC_TRACE}
        mp_tracevv('t=-1, w='+mpf_decimal(w,60)+#13#10);
      {$endif}
    end;
  end
  else begin
  ...
  for i:=1 to IMAX do begin
    ...
    mpf_mul(w,q,w);
    {$ifdef MPC_TRACE}
      mp_tracevv('e='+mpf_decimal(e,60)+#13#10);
      mp_tracevvv('w='+mpf_decimal(w,60)+#13#10);
    {$endif}
    pe := s_mpf_ldx(e);
