{Test program for MPArith/mp_real, (c) W.Ehrhardt 2007-2012}
...
(*
{---------------------------------------------------------------------------}
procedure mpf_pi_agm_standalone(var a: mp_float; pp: longint);
  {-calculate a = pi with Brent/Salamin AGM algorithm}
var
  j,prec: longint;
  b,t,y: mp_float;
begin
  ...
  repeat
    ...
    inc(j);
    s_mp_chs(y.mantissa);
  until (not s_mpf_incf(t,y)) or (mp_error<>MP_OKAY);
  {pi = a^2/t}
  mpf_sqr(a,a);
  mpf_div(a,t,a);
  mpf_chg_prec(a,pp);
...
const
  k1: extended = 545140134.0;
  k2: extended = 13591409.0;
  k3: extended = 1823176476672000.0;
var
  h: mp_float;
  a,b: mp_int;
  alpha,alpha2: extended;
  ...
  for i:=1 to imax do begin
    ...
    case k of
     ...
     19:  begin
            ...
            ts := 'mpf_arcsin';
            skip := mpf_cmp_mag_ext(a,1.0)=MP_GT;
            while skip do begin
              mpf_randomx(a);
              skip := mpf_cmp_mag_ext(a,1.0)=MP_GT;
            end;
     ...
     43:  begin
            ...
            ts := 'mpf_archav';
            skip := s_mpf_is_neg(a) or (mpf_cmp_ext(a,1)=MP_GT);
            while skip do begin
              mpf_randomx(a);
              skip := s_mpf_is_neg(a) or (mpf_cmp_ext(a,1)=MP_GT);
            end;
     ...
     46:  begin
            ...
            ts := 'mpf_lambertw';
            skip := mpf_cmp_ext(a,-0.367879441171)=MP_LT;
            while skip do begin
              mpf_randomx(a);
              skip := mpf_cmp_ext(a,-0.367879441171)=MP_LT;
            end;
...
begin
  writeln('Test of MP V', MP_VERSION, '   [mp_real]   (c) 2007-2012 W.Ehrhardt ');
