...
{$endif}
  ...
  PP1Cnt : word =  5000;        {number of iteration for Williams p+1}
  PM1Cnt : word = 20000;        {number of iteration for Pollard  p-1}
  CMax   : word = 32;           {number of curves for mp_ecm_factor  }
...
uses
  mp_base, mp_prime, mp_numth, mp_prng;
...
begin
  ...
  with ctx do begin
    ...
    if ((mp_cmp(n1, n2)=MP_EQ) or mp_iszero(n2)) and (PM1Cnt>0) and (not abort) then begin
      {try Pollard (p-1)}
      if trace then write('PP-1: ');
      mp_pollard_pm1(n1,n2,PM1Cnt);
      if trace then writeln;
    end;

    if ((mp_cmp(n1, n2)=MP_EQ) or mp_iszero(n2)) and (PP1Cnt>0) and (not abort) then begin
      {try Williams (p+1)}
      if trace then write('WP+1: ');
      mp_williams_pp1(n1,n2,PP1Cnt,3);
      if trace then writeln;
    end;
    ...
    if ((mp_cmp(n1, n2)=MP_EQ) or mp_iszero(n2)) and (CMax>3) and (not abort) then begin
      ...
      seed := 0;
      mp_ecm_factor(n1,n2,CMax div 4,ECM_C1Min,seed,k);
      if trace then writeln;
    end;
...
begin
  ...
  if mp_radix_size(num,10)>240 then begin
    writeln('Input number with more that 240 decimal digits');
    exit;
  end;
  abort := false;
  with ctx do begin
